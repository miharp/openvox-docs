...
## NAME
**puppet-filebucket** - Store and retrieve files in an OpenVox
filebucket
...
This filebucket client can operate in three modes, with only one mode
per call:
...
- Use a **find** command and **crontab** entry to retain only the last X
  days of file backups. For example:
...
:   Use the local filebucket. This uses the default configuration
    ...
    setting by default. If \'\--bucket\' is set, that path is used
    instead.
