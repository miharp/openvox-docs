...
String. A string used to match the catalog with the corresponding report that was issued during the same Puppet run. When running a Puppet agent off a cached catalog use `catalog_uuid` instead to match reports
with a catalog.
...
String. A string used to match the catalog with a Puppet job, intended for usage by the Puppet Enterprise Orchestration service. This field may be `null`.
...
String. The certname of the Puppet Server that sent the catalog to PuppetDB. This field may be `null`.
